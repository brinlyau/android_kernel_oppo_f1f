...
#Added by yixue.ge@bsp.drv, 2015-07-22 add for disable fastboot modem at release soft
ifneq ($(filter release cmcc cu ct,$(OPPO_BUILD_TYPE)),)
  CFLAGS_KERNEL += -DDISABLE_FASTBOOT_CMDS=1
endif

#Lycan.Wang@Prd.BasicDrv, 2015-10-27 Add for requirement from HW...
ifeq ($(OPPO_TARGET_DEVICE),MSM_15119)
  CFLAGS_KERNEL += -DIS_PROJECT_15119
endif

#Added by Tong.han@Bsp.group,2014-12-23,for select different config of console.
ifneq ($(SPECIAL_OPPO_CONFIG),1)
ifeq ($(filter release cmcc cmcctest cu ct cttest allnetcttest,$(OPPO_BUILD_TYPE)),)
