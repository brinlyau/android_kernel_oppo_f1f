...
static int __exit fpc1020_worker_destroy(fpc1020_data_t *fpc1020);
		...
		const char *buf, size_t count)
{
	...
	g_fpc1020->wait_abort = true;
	g_fpc1020->tee_interrupt_done = true;
	wake_up_interruptible(&g_fpc1020->g_irq_event);
				...
				size_t count, loff_t *ppos)
{
	...
	if (is_tee_driver) {
		//dev_dbg(&fpc1020->spi->dev, "%s : is_tee_driver", __func__);
		return -EINVAL;
	}
