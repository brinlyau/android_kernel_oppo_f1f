...
static void cm36686_lsensor_delay_work_handler(struct work_struct *work)
{
    ...
    if (sense > 0) {
        ...
        #ifdef VENDOR_EDIT
        //xiaohua.tian@EXP.Driver add for 15309 als, 2015-12-21
        if (is_project(OPPO_15109))
            report_lux = report_lux * 10 / 15;
        #endif /*VENDOR_EDIT*/
...
static void cm36686_psensor_initial_cmd(struct cm36686_info *lpi)
{
    ...
    _cm36686_I2C_Write_Word(lpi->slave_addr, PS_CANC,  lpi->ps_canc_val);
    ...
    _cm36686_I2C_Read_Word(lpi->slave_addr, PS_CANC,  &idReg3);

    printk(KERN_ERR"[PS]%s: PS_CONF1 = 0x%x, PS_CONF3 = 0x%x, PS_CANC = 0x%x!\n",__func__, idReg1, idReg2, idReg3);
...
static int cm36686_lightsensor_enable(struct cm36686_info *lpi)
{
    ...
    schedule_delayed_work(&lpi->ldwork,msecs_to_jiffies(90));
...
static ssize_t ps_canc_show(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    ret = sprintf(buf, "[PS][CM36686]PS_CANC = 0x%04x(%d)\n", lpi->ps_canc_val, lpi->ps_canc_val);

    printk("[PS][CM36686]PS_CANC: show value = 0x%04x(%d)\n", lpi->ps_canc_val, lpi->ps_canc_val);
...
static ssize_t ps_canc_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    ...
    printk("[PS][CM36686]PS_CANC: store value = 0x%04x(%d)\n", code, code);

    lpi->ps_canc_val = code;    
        ...
        struct cm36686_platform_data *pdata)
{
    ...
    rc = of_property_read_u32(np, "capella,ps_canc_val", &temp_val);
    if (rc) {
        dev_err(dev, "Unable to read ps_canc_val\n");
        return rc;
    } else {
        pdata->ps_canc_val = (u16)temp_val;
    }
        ...
        const struct i2c_device_id *id)
{
    ...
    #ifdef VENDOR_EDIT //xiaohua.tian@EXP.Driver modify for cm36686 add delay before probe, 2016-01-04
    msleep(15);
    #endif
    ...
    lpi->ps_canc_val = pdata->ps_canc_val;
