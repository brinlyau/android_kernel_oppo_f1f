...
static int lis3dh_acc_update_g_range(struct lis3dh_acc_data *acc, u8 new_g_range)
{
	...
	if (atomic_read(&acc->enabled)) {
		...
		dev_err(&acc->client->dev, "%s, CTRL_REG4 = 0x%x\n",__func__, buf[1]);
...
static int lis3dh_acc_update_odr(struct lis3dh_acc_data *acc, int poll_interval_ms)
{
	...
	j = i - 1;  // we need to choose a higher odr to get the correct data when open g-senosr in the beginning
	if(j<0) 
		j =0;

	config[1] = lis3dh_acc_odr_table[j].mask;
	...
	if (atomic_read(&acc->enabled)) {
		...
		dev_err(&acc->client->dev, "%s, CTRL_REG1 = 0x%x, poll_interval_ms = %d\n",__func__, config[1], poll_interval_ms);
	}
